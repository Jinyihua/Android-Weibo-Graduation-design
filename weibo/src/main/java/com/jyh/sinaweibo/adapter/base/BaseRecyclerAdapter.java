 ...
 * 通用适配器
 */
public abstract class BaseRecyclerAdapter<T extends Entity> extends RecyclerView.Adapter {
    ...
    *实现微博item列表界面
    * 包括三部分
    * 1、header（必须实现onLoadingHeaderCallBack.onCreateHeaderHolder）
    * 2、默认列表显示
    * 3、footer（默认加载recycler_footer_view）
    * */
    @Override
    ...
    //用于绑定数据
    //实现header数据绑定(实现onLoadingHeaderCallBack.onBindHeaderHolder)
    //实现默认列表的数据绑定（实现onBindDefaultViewHolder）
    //实现footer的数据绑定（）
    @Override
